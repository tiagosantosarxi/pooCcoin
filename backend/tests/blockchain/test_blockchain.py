...
def blockchain_three_blocks():
    blockchain = Blockchain()
    for i in range(3):
        blockchain.add_block([Transaction(Wallet(), 'deadly_recipient', i).to_json()])
    return blockchain
...
def test_replace_chain_bad_chain(blockchain_three_blocks):
    ...
    with pytest.raises(Exception, match='chain is invalid'):
        blockchain.replace_chain(blockchain_three_blocks.chain)
...
def test_is_valid_transaction_chain_duplicate_transacions(blockchain_three_blocks):
    transaction = Transaction(Wallet(), 'deadly_recipient', 1).to_json()
...
def test_is_valid_transaction_chain_bad_history_balance(blockchain_three_blocks):
    wallet = Wallet()
    bad_transaction = Transaction(wallet, 'deadly_recipient', 1)
